...
def compute_report(
    ...
    if upstream and not offline and source_allowed:
    ...
    if remote_state and remote_state.error:
        ...
        update_available = "unknown" if upstream else False
...
def update_lock_data(
    ...
    for report in reports:
        ...
        entry["current_tag"] = report.get("latest_tag_full") or entry.get("current_tag")
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    already_current = [
        report
        for report in reports
        if report["upstream"] and report["version_status"]["update_available"] is False
    ]
    ...
    lines = [
        ...
        f"- Update candidates: `{len(update_candidates)}`",
        f"- Risky updates: `{len(review_updates) + len(blocked_updates)}`",
        f"- Mode: `{'offline' if payload['offline'] else 'online'}`",
